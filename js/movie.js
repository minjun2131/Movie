...
// 모달 보여주는 함수
// 모달을 여는 순간 localStorage에서 id가 있으면 북마크 해제 버튼을 만들고, 아니면 북마크 추가 버튼을 만든다 
function showModal(movieArray, thisMovieId) {
    ...
    // 북마크 제거 추가 로직
    const bookMarkChk = localStorage.getItem('dataId-list')
    if (bookMarkChk.includes(thisMovieId)) {
...
bookMarkBtn.addEventListener('click',function (event) {
    ...
    // 현재 로컬스토리지에 클릭한 아이디가 이미 포함되어 있으면
